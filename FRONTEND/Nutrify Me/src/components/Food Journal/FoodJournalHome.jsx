...
function FoodJournalHome() {
  ...
  const [BPRange, setBPRange] = useState(["Normal Blood Pressure", "#4DDA5B", "Maintain a Balanced Diet"]);
  ...
  const [caloriesConsidered, setCaloriesConsidered] = useState(400);
  const [caloriesConsidered1, setCaloriesConsidered1] = useState(2000);
      ...
      try {
        ...
        switch (category) {
          case "Normal Blood Pressure":
            link = `q=&dishType=main&calories=${caloriesConsidered1}&`;
            break;
          case "Elevated Blood Pressure":
            link = `q=&dishType=main&diet=low-sodium&calories=${caloriesConsidered1}&`;
            break;
          case "Stage 1 Hypertension":
            link = `q=&dishType=main&health=DASH&diet=low-sodium&calories=${caloriesConsidered1}&`;
            break;
          case "Stage 2 Hypertension":
            link = `q=&dishType=main&health=DASH&diet=low-sodium&calories=${caloriesConsidered}-${caloriesConsidered1}&`;
            break;
          // default:
          //   link = `q=&dishType=main&calories=${caloriesConsidered}-${caloriesConsidered1}&`;
          //   break;
        }
        ...
        arrayType.map((item) =>
          ...
          console.log(   `https://api.edamam.com/search?app_id=${EDAMAM_APP_ID}&app_key=${EDAMAM_APP_KEY}&from=1&to=100&` +
            link +
            `mealType=${item}`)
        );
...
console.log(responses)
  ...
  const getCurrentData = async (day) => {
    AxiosInstance.get(`profiling`).then((respon) => {
      setProfilingData(
        respon.data.find((item) => item.user_id_id === loggedInUser.user_id)
      );
    });
    ...
    await AxiosInstance.get(`journalentry`).then((res) => {
      ...
      if (currentDate) {
        ...
        AxiosInstance.get(`foodentry`).then((respo) => {
          setCurrentFoodData(
            respo.data?.filter(
              (item) => item.journal_id == currentDate.journal_id
            )
          );
          ...
          try {
            AxiosInstance.get(`foodentry`).then((res) => {
              setCurrentFoodData(
                res.data.filter(
                  (item) => item.journal_id_id === currentDate.journal_id
                )
              );
            });
          } catch (error) {
        ...
        if (entries) {
          ...
          try {
            AxiosInstance.get(`foodentry`).then((res) => {
              setCurrentFoodData(
                res.data.filter(
                  (item) => item.journal_id_id === entries[0].journal_id
                )
              );
            });
          } catch {
          ...
          AxiosInstance.get(`profiling`).then((respon) => {
            setCurrentJournalData(
              respon.data.find(
                (item) => item.user_id_id === loggedInUser.user_id
              )
            );
          });
        }
        ...
        getBPRange(
          parseInt(
            res.data?.filter(
              ...
              // Assuming 'date' is a property of each item
  ...
  const [randomNumberbreakfast1, setRandomNumberbreakfast1] = useState(0);
  const [randomNumberlunch1, setRandomNumberlunch1] = useState(0);
  const [randomNumbersnack1, setRandomNumbersnack1] = useState(0);
  const [randomNumberdinner1, setRandomNumberdinner1] = useState(0);
  useEffect(() => {
        ...
        AxiosInstance.post(`journalentry/`, {
          ...
          try {
            AxiosInstance.post(`foodentry/`, {
              ...
              setPotassiumLvalue(0);
              setLunchFood("")
              setLunchFoodImage("/images/food.png")
              ...
              setPotassiumSvalue(0);
              setSnackFood("")
              setSnackFoodImage("/images/food.png")
              ...
              setPotassiumDvalue(0);
              setDinnerFood("")
              setDinnerFoodImage("/images/food.png")
...
const [notFound, setNotFound] = useState(false)
  ...
  const handleSuggestionClick = (suggestion, index, type) => {
    ...
    if (type === "breakfast") {
      ...
      setPotassiumBvalue(parseInt(tempData[index].digest[7].total / tempData[index].yield));
      setSodiumBvalue(parseInt(tempData[index].digest[4].total / tempData[index].yield ));
    } else if (type === "lunch") {
  ...
  const handleSuggestionClickLunch = (suggestion, index, type) => {
    ...
    setCaloriesLvalue(
      parseInt(tempDataLunch[index].calories / tempDataLunch[index].yield)
    );
    ...
    setPotassiumLvalue(parseInt(tempDataLunch[index].digest[7].total / tempDataLunch[index].yield));
    setSodiumLvalue(parseInt(tempDataLunch[index].digest[4].total / tempDataLunch[index].yield));
  ...
  const handleSuggestionClickSnack = (suggestion, index, type) => {
    ...
    setCaloriesSvalue(
      parseInt(tempDataSnack[index].calories / tempDataSnack[index].yield)
    );
    ...
    setPotassiumSvalue(parseInt(tempDataSnack[index].digest[7].total / tempDataSnack[index].yield));
    setSodiumSvalue(parseInt(tempDataSnack[index].digest[4].total / tempDataSnack[index].yield));
  ...
  const handleSuggestionClickDinner = (suggestion, index, type) => {
    ...
    setCaloriesDvalue(
      parseInt(tempDataDinner[index].calories / tempDataDinner[index].yield)
    );
    ...
    setPotassiumDvalue(parseInt(tempDataDinner[index].digest[7].total / tempDataDinner[index].yield));
    setSodiumDvalue(parseInt(tempDataDinner[index].digest[4].total / tempDataDinner[index].yield));
  ...
  const fetchMealSuggestionsHypertension = useCallback(
    _.debounce(async (query) => {
      ...
      try {
        ...
        setTempDataHypertension(response.data.hits.map((hit) => hit.recipe));
        setSuggestionsHypertension(
          response.data.hits.map((hit) => hit.recipe.label)
        );
      } catch (error) {
        console.error("Error fetching meal suggestions:", error);
      } finally {
  ...
  const getDietRecommendations = (Hyper) => {
    ...
    if (Hyper) {
      ...
      if (
        BPRange[0] === "Stage 1 Hypertension" ||
        BPRange[0] === "Stage 2 Hypertension"
      ) {
        // User with Hypertension
        if (totalSodium > 1500) {
          pushData.push(
            "Your sodium intake is too high. Consider reducing foods high in salt to help lower your blood pressure. "
          );
        ...
        if (totalPotassium < 3500) {
          pushData.push(
            "Your potassium intake is low. Consider adding potassium-rich foods like bananas, leafy greens, and avocados to your diet. "
          );
        ...
        pushData.push(
          "Keep focusing on a balanced diet. Make sure to include enough protein, healthy fats, and complex carbs for overall well-being. "
        );
  ...
  const generateWeeklyReport = async (day) => {
    ...
    try {
      ...
      const journalResponse = await AxiosInstance.get(`journalentry`);
      ...
      const foodResponse = await AxiosInstance.get(`foodentry`);
      let foodData = [];
      journalData.forEach((journal) => {
        ...
        relatedFoodEntries.map((item) => foodData.push(item));
      ...
      if (avgSystolic >= 130 || avgDiastolic >= 80) {
        reportMessage +=
          "Your average blood pressure this week is above normal. Please consider reducing sodium intake and consulting your healthcare provider. ";
        message =   "Your average blood pressure this week is above normal. Please consider reducing sodium intake and consulting your healthcare provider. ";
       
        } else {
        reportMessage +=
          "Your average blood pressure is within normal range. Keep maintaining a healthy diet and staying active. ";
          message = "Your average blood pressure is within normal range. Keep maintaining a healthy diet and staying active. ";
      ...
      if (totalSodium > 10500) {
        reportMessage +=
          "Your sodium intake this week has been high. Consider reducing the intake of processed and salty foods. ";
          message1 =  "Your sodium intake this week has been high. Consider reducing the intake of processed and salty foods. ";
         
        }

      if (totalPotassium < 24500) {
        reportMessage +=
          "Consider adding more potassium-rich foods like bananas, spinach, and avocados to help maintain a healthy sodium-potassium balance. ";
          message1 =   "Consider adding more potassium-rich foods like bananas, spinach, and avocados to help maintain a healthy sodium-potassium balance. ";
...
<Typography> Weekly Report</Typography>
          <Typography sx = {{color: "#E66253"}}>
          Average Blood Pressure: {" "}
          {Math.floor(avgSystolic)} / {Math.floor(avgDiastolic)} mmHg
          </Typography>
          <Typography> {message}</Typography>
          <Typography>
        
        <Typography sx = {{color: "#E66253"}}>Dietary Summary</Typography>
          <Grid container spacing = {2} sx = {{mt: "1%"}}>
          <Grid xs = {2}>  Total Sodium:
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalSodium} {" "}g</Typography>
          </Grid>
          <Grid xs = {2}>
          Total Calories:
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalCalories}{" "}g</Typography>
          </Grid>
          <Grid xs = {2}>
          Total Fat:
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalFat}{" "}g</Typography>
          </Grid>
          <Grid xs = {2}>
          Total Protein:
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalProtein}{" "}g</Typography>
          </Grid>
          <Grid xs = {2}>
          Total Carbs:
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalCarbs}{" "}g</Typography>
          </Grid>
          <Grid xs = {2}>
            Total Potassium: 
          <Typography sx = {{mr: "30%", ml: "30%", borderRadius: 4, color: "#ffffff", background: "#E66253"}}>
          {totalPotassium}{" "}g</Typography>
          </Grid>
                                ...
                                <Grid xs={4}>
                                  ...
                                  >
                                    <TextField
                                      ...
                                      error={
                                        errors.breakfast_food ? true : false
                                      }
                                      label="Food Eaten:"
                                      variant="filled"
                                      size="small"
                                      ...
                                      >
                                        ...
                                        <Grid item xs>
                                          <Slider
                                            ...
                                            value={
                                              typeof potassiumBvalue ===
                                              "number"
                                                ? potassiumBvalue
                                                : 0
                                            }
                                            onChange={
                                              handlePotassiumBSliderChange
                                            }
                                            aria-labelledby="input-slider"
                                            sx={{ color: secondaryColor }}
                                          />
                                ...
                                <Grid xs={4}>
                                  ...
                                  >
                                    <TextField
                                      ...
                                      value={lunchFood}
                                      onChange={(e) =>
                                        handleInputChangesLunch(e, "lunch")
                                      }
                                      placeholder="Type to search for meals..."
                                      sx={{
                                        //  mr: 2,
                                    ...
                                    {suggestionsLunch.length > 0 ? (
                                      ...
                                      >
                                        {suggestionsLunch.map(
                                          (suggestion, index) => (
                                            <ListItem
                                              button
                                              key={index}
                                              onClick={() =>
                                                handleSuggestionClickLunch(
                                                  suggestion,
                                                  index,
                                                  "lunch"
                                                )
                                    ...
                                    ) : suggestionsLunch.length <= 0 && notFound1 ? (
                                      <>
                                       <List
                                        style={{
                                          position: "absolute",
                                      ...
                                      >
                                        ...
                                        <Grid item xs>
                                          <Slider
                                            ...
                                            value={
                                              typeof potassiumLvalue ===
                                              "number"
                                                ? potassiumLvalue
                                                : 0
                                            }
                                            onChange={
                                              handlePotassiumLSliderChange
                                            }
                                            aria-labelledby="input-slider"
                                            sx={{ color: secondaryColor }}
                                          />
                                ...
                                <Grid xs={4}>
                                  ...
                                  >
                                    <TextField
                                      ...
                                      value={snackFood}
                                      onChange={(e) =>
                                        handleInputChangesSnack(e, "lunch")
                                      }
                                      placeholder="Type to search for meals..."
                                      sx={{
                                        //  mr: 2,
                                    ...
                                    {suggestionsSnack.length > 0 ? (
                                      ...
                                      >
                                        {suggestionsSnack.map(
                                          (suggestion, index) => (
                                            <ListItem
                                              button
                                              key={index}
                                              onClick={() =>
                                                handleSuggestionClickSnack(
                                                  suggestion,
                                                  index,
                                                  "lunch"
                                                )
                                    ...
                                    ) : suggestionsSnack.length <= 0 && notFound2 ? (
                                      <>
                                       <List
                                        style={{
                                          position: "absolute",
                                      ...
                                      >
                                        ...
                                        <Grid item xs>
                                          <Slider
                                            ...
                                            value={
                                              typeof potassiumSvalue ===
                                              "number"
                                                ? potassiumSvalue
                                                : 0
                                            }
                                            onChange={
                                              handlePotassiumSSliderChange
                                            }
                                            aria-labelledby="input-slider"
                                            sx={{ color: secondaryColor }}
                                          />
                                ...
                                <Grid xs={4}>
                                  ...
                                  >
                                    <TextField
                                      ...
                                      value={dinnerFood}
                                      onChange={(e) =>
                                        handleInputChangesDinner(e, "lunch")
                                      }
                                      placeholder="Type to search for meals..."
                                      sx={{
                                        //  mr: 2,
                                    ...
                                    {suggestionsDinner.length > 0 ? (
                                      ...
                                      >
                                        {suggestionsDinner.map(
                                          (suggestion, index) => (
                                            <ListItem
                                              button
                                              key={index}
                                              onClick={() =>
                                                handleSuggestionClickDinner(
                                                  suggestion,
                                                  index,
                                                  "lunch"
                                                )
                                    ...
                                    ) : suggestionsDinner.length <= 0 && notFound3 ? (
                                      <>
                                       <List
                                        style={{
                                          position: "absolute",
                                      ...
                                      >
                                        ...
                                        <Grid item xs>
                                          <Slider
                                            ...
                                            value={
                                              typeof potassiumDvalue ===
                                              "number"
                                                ? potassiumDvalue
                                                : 0
                                            }
                                            onChange={
                                              handlePotassiumDSliderChange
                                            }
                                            aria-labelledby="input-slider"
                                            sx={{ color: secondaryColor }}
                                          />
            ...
            <Grid container spacing={2} sx={{ mb: "1px" }}>
              ...
              <Grid xs={6}>
                ...
                >
                  ...
                  <Grid xs={6} md={2}>
                    {" "}
                    <img width = "10%" height= "50%"  src="/images/sodiumorange.png" /> {meal.sodium}g sodium
                  </Grid>
                  <Grid xs={6} md={2}>
                    {" "}
                    <img  width = "10%" height= "50%" src="/images/potassiumorange.png" /> {meal.potassium}g potassium
                  </Grid>
                </Grid>
              </Grid>
              <Grid xs={2}>
      ...
      <Box sx={{ ml: "3%", mr: "6%" }}>
        <Accordion sx={{ border: 3, borderColor: "#898246" }}>
          <AccordionSummary
            ...
            id="panel1-header"
          >
            <img src="/images/day.png" width="40px" height="40px" />
            <Typography
              sx={{ mt: "1%", ml: "5%", fontWeight: "bold", color: "#99756E" }}
            >
              Meal Recommendation of the Day
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            <center>
              ...
              <Typography sx={{ ml: "20%", mr: "20%" }}>
                {dietRecommendation[1]}
              </Typography>
            </center>
            <hr />
            <Box
              ...
              <Box sx={{ ml: 1, mr: 0 }}>
                <Grid container spacing={2} sx={{ mt: "2%" }}>
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[0]?.meals[randomNumberbreakfast]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    <Link
                                                to={mealBPRecommendations[0]?.meals[randomNumberbreakfast]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[1]?.meals[randomNumberlunch]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {
                      mealBPRecommendations[1]?.meals[randomNumberlunch]?.label
                    }{" "}
                    <br/>

                                <Link
                                                to={mealBPRecommendations[1]?.meals[randomNumberlunch]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[2]?.meals[randomNumbersnack]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {
                      mealBPRecommendations[2]?.meals[randomNumbersnack]?.label
                    }{" "}
                    <br/>

<Link
                                                to={mealBPRecommendations[2]?.meals[randomNumbersnack]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[3]?.meals[randomNumberdinner]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {
                      mealBPRecommendations[3]?.meals[randomNumberdinner]?.label
                    }{" "}
                    <br/>

<Link
                                                to={mealBPRecommendations[3]?.meals[randomNumberdinner]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
      ...
      <Box sx={{ ml: "3%", mr: "6%", mt: 7 }}>
        <Accordion sx={{ border: 3, borderColor: "#898246" }}>
          <AccordionSummary
            ...
            id="panel1-header"
          >
            <img src="/images/week.png" width="40px" height="40px" />
            <Typography
              sx={{ mt: "1%", ml: "5%", fontWeight: "bold", color: "#99756E" }}
            >
          ...
          <AccordionDetails>
            <center>
              <Typography sx={{ color: "#99756E", fontWeight: "bold", mx: "10%" }}>
              ...
              <Box sx={{ ml: 1, mr: 0 }}>
                <Grid container spacing={2} sx={{ mt: "2%" }}>
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[0]?.meals[randomNumberbreakfast1]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    >
                      {
                        mealBPRecommendations[0]?.meals[randomNumberbreakfast1]?.label
                      }
                    </Typography>

                    <Link
                                                to={mealBPRecommendations[0]?.meals[randomNumberbreakfast1]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[1]?.meals[randomNumberlunch1]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {mealBPRecommendations[1]?.type}: <br />
                    {
                      mealBPRecommendations[1]?.meals[randomNumberlunch1]?.label
                    }{" "}
                    <br/>

                                <Link
                                                to={mealBPRecommendations[1]?.meals[randomNumberlunch1]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[2]?.meals[randomNumbersnack1]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {mealBPRecommendations[2]?.type}: <br />
                    {
                      mealBPRecommendations[2]?.meals[randomNumbersnack1]?.label
                    }{" "}
                    <br/>

<Link
                                                to={mealBPRecommendations[2]?.meals[randomNumbersnack1]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
                  ...
                  <Grid xs={3}>
                    <img
                      src={
                        mealBPRecommendations[3]?.meals[randomNumberdinner1]?.image
                      }
                      width="60%"
                      height="60%"
                    />
                    ...
                    {mealBPRecommendations[3]?.type}: <br />
                    {
                      mealBPRecommendations[3]?.meals[randomNumberdinner1]?.label
                    }{" "}
                    <br/>

<Link
                                                to={mealBPRecommendations[3]?.meals[randomNumberdinner1]?.url}
                                                target="_blank"
                                              >
                                                <Button
                                                  sx={{
