  ...
  const getData = async () => {
    await AxiosInstance.get(`generatedmeal`).then((res) => {
      setGeneratedMealPlans(
        res.data.filter((item) => item.user_id == location.state.item.user_id)
      );
    });

    await AxiosInstance.get(`order`).then((res) => {
      let temp = res.data.filter(
        (item) => item.user_id == location.state.item.user_id
      );
          ...
          try {
            AxiosInstance.get(`shopmeal`).then((respo) => {
              ...
              tempOrder.map(
                (item) => (
                  console.log(
                    respo.data.filter((items) => items.mealplan_id === item)
                  ),
  ...
  const getJournalData = async (day) => {
    ...
    await AxiosInstance.get(`journalentry`).then((res) => {
      setJournalEntry(
        res.data.filter(
          (item) =>
            item.date == day && item.user_id == location.state.item.user_id
        )
      );
  ...
  return (
    ...
    >
      ...
      <Box sx = {{ml: "20%"}}>
        ...
        <Grid xs = {6}>
            <img src = {location.state.item.image} width="30%" height="70%"/>
        </Grid>
        </Grid>
...
<Box sx = {{ml: 5, mr:7}}>
      ...
      <Box sx = {{border: 1, ml: 8, mr: 8, mb: 4, py: 5}}>
       
     

     <br/>
      {foodEntry.length > 0 ? ( <>
        <Typography sx = {{ml: 6, display: "flex", justifyContent: "flex-start", color: "#99756E", fontWeight:"bold", fontSize: "1.5em"}}> {journalEntry[0]?.title}</Typography>
     <Typography sx = {{ml: 6, display: "flex", justifyContent: "flex-start", color: "#99756E", fontSize: "1em"}}> {journalEntry[0]?.date}</Typography>
     <Typography sx = {{ml: 6, display: "flex", justifyContent: "flex-start", color: "#99756E", fontSize: "1em"}}>
    Overall Details: 
    </Typography>
    <Grid container spacing = {2} sx ={{mt: 1.5, ml: "10%", display: "flex",mb: 3, justifyContent: "flex-start"}}>
    <Grid xs = {4} >
    <Typography sx = {{display: "flex", justifyContent: "flex-start"}}> {Math.round(calories)} calories </Typography>
        <Typography sx = {{display: "flex", justifyContent: "flex-start"}}>
    {fat}g fat</Typography>
    </Grid>
<Grid xs = {6}>
    
<Typography sx = {{display: "flex", justifyContent: "flex-start"}}>{carbs}g carbs </Typography>
<Typography sx = {{display: "flex", justifyContent: "flex-start"}}>{protein}g protein </Typography>
</Grid>
</Grid>
       { foodEntry
          .sort((a, b) => {
          ...
          .map((meal, index) => (
            <Box sx = {{ml: 8}}>
<Typography sx = {{display: "flex", justifyContent: "flex-start", color: "#99756E", fontWeight:"bold", fontSize: "1.5em"}}> 
{meal.type}:{meal.food}</Typography>
<Grid container spacing = {2} sx ={{mt: 1.5, ml: 3}}>
    <Grid xs = {4} >
    <Typography sx = {{display: "flex", justifyContent: "flex-start"}}> {meal.calories} calories </Typography>
        <Typography sx = {{display: "flex", justifyContent: "flex-start"}}>
    {meal.fat}g fat</Typography>
    </Grid>
<Grid xs = {6}>
    
<Typography sx = {{display: "flex", justifyContent: "flex-start"}}>{meal.carbs}g carbs </Typography>
<Typography sx = {{display: "flex", justifyContent: "flex-start"}}>{meal.protein}g protein </Typography>
</Grid>
</Grid>
...
export default NutritionistPatientRecords;
